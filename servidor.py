...
SEQ_PATH = os.path.join(BASE_DIR, "protocolo.dat")
...
# ---------- Protocolo (contador simples em arquivo) ----------
def gerar_protocolo():
    ...
    try:
        with open(SEQ_PATH, "r", encoding="utf-8") as f:
            dado = json.load(f)
        if dado.get("ano") == ano:
            seq = int(dado.get("seq", 0)) + 1
    except Exception:
        pass
    with open(SEQ_PATH, "w", encoding="utf-8") as f:
        json.dump({"ano": ano, "seq": seq}, f)
    return f"MID-{ano}-{seq:06d}"
